...
- Start:  28.11.2025
- End:  28.11.2025
- Test environment details (OS, runtime, DB, browsers): Windows, Local web server, Firefox

**Assumptions & constraints:**  
- Limited to ZAP automated scanning + light manual verification
- Application accessible only on local network
...
|------|-----------|----------|--------------|------------------|
| F-01 | 🟠 Medium | Absence of Anti-CSRF Tokens | Forms or endpoints handling state-changing actions do not include CSRF tokens, exposing users to CSRF attacks. | ZAP Alert: Absence of Anti-CSRF Tokens |
| F-02 | 🟠 Medium | Content Security Policy (CSP) Header Not Set | Missing CSP header makes the application more vulnerable to XSS and content injection. | ZAP Alert: CSP Header Not Set (3) |
| F-03 | 🟠 Medium | Missing Anti-Clickjacking Header | The app lacks X-Frame-Options or frame-ancestors, allowing clickjacking attacks. | ZAP Alert: Missing Anti-clickjacking Header (3) |
| F-04 | 🟡 Low | X-Content-Type-Options Header Missing | Without nosniff, browsers may interpret content as another MIME type, enabling certain attacks. | ZAP Alert: X-Content-Type-Options Header Missing (6) |
| F-05 | 🟠 Medium | Format String Error | An endpoint may improperly process format specifiers (e.g., %s, %d), indicating unsafe input handling. | ZAP Alert: Format String Error |
| F-06 | 🔵 Info | Suspicious User Agent Behavior Detected | Automated fuzzer detected variations in user-agent handling; may reveal inconsistent server behavior. | ZAP Alert: User Agent Fuzzer (12) |
| F-07 | 🔴 High | Path Traversal | The application is vulnerable to directory traversal attempts, allowing an attacker to access files outside the intended directory. | ZAP detected responses that indicate attempts to access unintended file paths may be partially or fully successful. |
| F-08 | 🔴 High | SQL Injection | User-supplied input is not properly sanitized before being included in SQL queries. This allows an attacker to inject malicious SQL code, potentially leading to authentication bypass, data leakage, or complete compromise of the database. | SQL injection vectors were flagged during fuzzing (e.g., ' OR '1'='1). |
---
